...
# 1. Исправляем unlockAchievement
...
if idx_fn != -1:
    ...
    if idx_chan != -1 and idx_send != -1:
        ach_new = """if (channel) {
          ...
          try {
            """
        content = content[:idx_chan] + ach_new + content[idx_send:]
        print("✓ 1. unlockAchievement успешно обновлен")

# 2. Исправляем синтаксическую ошибку на строке 5300 (хвост после map)
...
if idx_map != -1 and idx_ready != -1:
    map_block_new = """const existingMap = guild.commands.cache.find((c: any) => c.name === 'map');
        if (existingMap) {
  ...
  """
    content = content[:idx_map] + map_block_new + content[idx_ready:]
    print("✓ 2. Хвост на строке 5300 исправлен")

# 3. Заменяем системные уведомления, если остались кракозябры
replacements = {
...
for old, new in replacements.items():
    if old in content:
        ...
        rep_count += 1
print(f"✓ 3. Системные уведомления проверены и обновлены ({rep_count})")

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)
print("✓ Файл collector/src/collector.ts успешно сохранен!")
